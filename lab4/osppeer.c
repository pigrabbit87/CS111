...
//#define MAXFILESIZ	1000000000	// Maximum download size 1GB
...
taskbufresult_t write_from_taskbuf(int fd, task_t *t)
{
	...
	if (amt == -1 && (errno == EINTR || errno == EAGAIN
		...
		t->head += amt;
		t->total_written += amt;
		/*if(t->total_written > TASKBUFSIZ){	
			error("Number of bytes written is larger than buffer size");
			t->total_written = TASKBUFSIZ;
		}*/
		return TBUF_OK;
	}
}
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	// To prevent a peer trying to send huge file over and DDoS our file system
	// set the maximum file size one peer can transfer, define in the beginning of the file
	/*if (t->total_writte > MAXFILESIZ) {
		error("Your file size is too big. Cannot be downloaded");
		goto try_again;
	}*/
	// Perhaps not effective because it is possible to torrent a really large file
	...
	// recursive call
	if(!evil_mode)                      //if evil mode, don't pop peer 
		task_pop_peer(t);
	task_download(t, tracker_task);
}
...
static void task_upload(task_t *t)
{
	...
	//TASK 2
	//check if filename passed in is in current directory before opening
	
	//get current directory
	char currDirPathBuf[PATH_MAX];
	char* currDirPath = getcwd(currDirPathBuf, PATH_MAX);
	...
	char* filePath = realpath(t->filename, filePathBuf);
	
	if(filePath == NULL){
		error("Error finding path of file\n");
		goto exit;
	}
	
	//compare current directory path with filename path
	if(strncmp(currDirPath, filePath, strlen(currDirPath)) != 0){
		error("Error accessing file outside of current directory\n");
		goto exit;
	}	
	...
	for (; argc > 1; argc--, argv++)
	{
		if ((t = start_download(tracker_task, argv[1]))){
			...
			if(child_pid < 0){
				error("Cannot fork");
				exit(0);
			} else if (child_pid == 0){ //child downloads
				if(evil_mode){
					while(1)
						task_download(t, tracker_task);
				} else {
